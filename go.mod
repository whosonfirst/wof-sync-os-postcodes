module github.com/tomtaylor/wof-sync-os-postcodes

go 1.12

require (
	github.com/aaronland/go-artisanal-integers-proxy v0.2.3
	github.com/aaronland/go-pool v0.0.0-20191128211702-88306299c758
	github.com/hashicorp/go-multierror v1.0.0 // indirect
	github.com/smartystreets/scanners v1.0.1
	github.com/tidwall/gjson v1.5.0
	github.com/tidwall/sjson v1.0.4
	github.com/whosonfirst/go-whosonfirst-crawl v0.1.1-0.20190822225219-b571d777dc43 // indirect
	github.com/whosonfirst/go-whosonfirst-export v0.3.1
	github.com/whosonfirst/go-whosonfirst-geojson-v2 v0.12.2
	github.com/whosonfirst/go-whosonfirst-id-proxy v0.0.0-20200309194549-fa6ef0bb33b4
	github.com/whosonfirst/go-whosonfirst-index v0.1.3
	github.com/whosonfirst/go-whosonfirst-sqlite v0.1.1 // indirect
	github.com/whosonfirst/go-whosonfirst-uri v0.1.0
	go.uber.org/atomic v1.6.0 // indirect
	golang.org/x/crypto v0.0.0-20190927123631-a832865fa7ad // indirect
	golang.org/x/net v0.0.0-20190926025831-c00fd9afed17 // indirect
	golang.org/x/sync v0.0.0-20190911185100-cd5d95a43a6e
	golang.org/x/sys v0.0.0-20190927073244-c990c680b611 // indirect
)

replace github.com/whosonfirst/go-whosonfirst-crawl => github.com/tomtaylor/go-whosonfirst-crawl v0.1.1-0.20190929185029-a65c11bebf0f

replace github.com/tidwall/gjson => github.com/tidwall/gjson v1.3.5
